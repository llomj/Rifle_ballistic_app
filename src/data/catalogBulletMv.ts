...
import bulletMuzzleVelocityById from './bulletMuzzleVelocityById.json';

/** Per catalog bullet id (m/s). Generated by `node scripts/generate-bullet-mv.mjs` — see averagespeed.md. */
const MV_BY_BULLET_ID = bulletMuzzleVelocityById as Record<string, number>;
...
export function getCatalogMuzzleVelocityMps(bullet: BulletCatalogItem): number {
  const fromTable = MV_BY_BULLET_ID[bullet.id];
  if (typeof fromTable === 'number' && Number.isFinite(fromTable) && fromTable > 0) {
    return fromTable;
  }
  if (
